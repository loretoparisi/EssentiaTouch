...
class TempoTapDegara : public Algorithm {
 ...
 private:
  ...
  const int _numberCombs = 4;
  const Real _frameDurationODF = 5.944308390022676;
  Real _sampleRateODF;
  int _hopSizeODF;
  ...
  const Real _alpha = 0.5; // decoding weighting parameter
  const Real _sigma_ibi = 0.025; // std of the inter-beat interval pdf,
